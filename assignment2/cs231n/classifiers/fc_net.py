...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                self.params['W' + str(i + 1)] = np.random.randn(hidden_dims[i - 1], num_classes)
    ...
    def loss(self, X, y=None):
        ...
        pass
        layer_input = X
        bn_cache = {} # 用字典保存求导所需要的中间值
        affine_cache = {}
        dp_cache = {}
        relu_cache = {}
        for i in range(self.num_layers - 1):
            if self.normalization == "batchnorm":
                layer_input, affine_cache[i] = affine_forward(layer_input, 
                    self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
                layer_input, bn_cache[i] = batchnorm_forward(layer_input, 
                    self.params['gamma' + str(i + 1)], self.params['beta' + str(i + 1)], self.bn_params[i])
                layer_input, relu_cache[i] = relu_forward(layer_input)
            elif self.normalization == "layernorm":
                layer_input, affine_cache[i] = affine_forward(layer_input, 
                    self.params['W' + str(i + 1)], self.params['b' + str(i + 1)])
                layer_input, bn_cache[i] = layernorm_forward(layer_input, 
                    self.params['gamma' + str(i + 1)], self.params['beta' + str(i + 1)], self.bn_params[i])
                layer_input, relu_cache[i] = relu_forward(layer_input)
            else:
        ...
        scores, affine_cache[self.num_layers] = affine_forward(layer_input, 
        ...
        dx, dw, db = affine_backward(dout, affine_cache[self.num_layers])
        ...
        for i in range(self.num_layers - 1)[::-1]:
            ...
            if self.normalization == "batchnorm":
                ...
                dout, dgamma, dbeta = batchnorm_backward_alt(dout, bn_cache[i])
                dout, dw, db = affine_backward(dout, affine_cache[i])
                # batchnorm_backward(dout, cache)
            elif self.normalization == "layernorm":
                dout = relu_backward(dout, relu_cache[i])
                dout, dgamma, dbeta = layernorm_backward(dout, bn_cache[i])
                dout, dw, db = affine_backward(dout, affine_cache[i])
            else:
                dout, dw, db = affine_relu_backward(dout, affine_cache[i])
