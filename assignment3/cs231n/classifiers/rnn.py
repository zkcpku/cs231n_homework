...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        pass
        # forward:hidden_state & embedding -> rnn -> fc -> softmax
        ...
        if self.cell_type == "rnn":
            ...
            rnn_out, rnn_cache = lstm_forward(word_embedding_out, affine_out, Wx, Wh, b)
        ...
        if self.cell_type == "rnn":
            dword_embedding_out,  daffine_out, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(drnn_out, rnn_cache)
        else:
            dword_embedding_out, daffine_out, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(drnn_out, rnn_cache)
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            ...
            if self.cell_type == "rnn":
                ...
                next_h,prev_c, _ = lstm_step_forward(prev_word_embed, prev_h, prev_c, Wx, Wh, b)

            # 不使用temporal_affine，因为需要一个个单词生成
            word_affine_out, _ = affine_forward(next_h, W_vocab, b_vocab)

            # 这里采用的策略是从中找出最大的，也可以用前k大随机
            captions[:,i] = list(np.argmax(word_affine_out, axis = 1))
